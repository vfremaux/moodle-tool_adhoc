...
class manager
{
     ...
     */
    public static function run_tasks($records, $ignoreblocking = false, $verbose = false) {
        global $CFG, $DB;
        ...
        foreach ($records as $record) {
            ...
            try {
                ...
                if (!$lock = $cronlockfactory->get_lock('adhoc_' . $record->id, 10)) {
                    cli_problem('Cannot obtain task lock '.$task->get_name());
                    continue;
                }
                ...
                if ($task->is_blocking()) {
                    if (!$cronlock = $cronlockfactory->get_lock('core_cron', 10)) {
                        cli_problem('Cannot obtain cron lock form task '.$task->get_name());
                        continue;
                    }
